...
RepositoryPostgres::RepositoryPostgres(userver::storages::postgres::ClusterPtr pg_cluster)
    : pg_cluster_(std::move(pg_cluster)) {}

std::optional<std::string> RepositoryPostgres::InsertUrl(const std::string& short_url, 
                                                         const std::string& original_url){
    userver::storages::postgres::Transaction transaction = pg_cluster_->Begin("transaction_insert_url", 
                                                                              userver::storages::postgres::ClusterHostType::kMaster, {});
    auto res = transaction.Execute(
        "INSERT INTO urls (short_url, original_url) VALUES ($1, $2) "
        "ON CONFLICT (short_url) DO NOTHING ", short_url, original_url);
    if (res.RowsAffected()) {
        transaction.Commit();
    ...
    res = transaction.Execute(
        "SELECT short_url FROM urls WHERE short_url = $1", short_url);
